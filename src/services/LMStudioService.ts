...
export class LMStudioService {
   ...
   * Phase 9: Includes timeout for reliability (15 second max)
   */
  private readonly REQUEST_TIMEOUT_MS = 15000; // 15 seconds max
  ...
  private async sendRequest(): Promise<{
    ...
    try {
      ...
      if (this.mode === 'proxy') {
        ...
        // Phase 9: Timeout handled by Promise.race
        const timeoutPromise = new Promise<never>((_, reject) => {
          setTimeout(() => reject(new Error('LLM request timed out (15s)')), this.REQUEST_TIMEOUT_MS);
        });
        ...
        try {
          ...
          if (error instanceof Error && error.name === 'AbortError') {
            throw new Error('LLM request timed out (15s). The model may be overloaded or not responding.');
          }
          throw error;
        }
      }
