...
declare global {
  interface Window {
    jarvisHost?: {
      ...
      // Phase 5: Device Pairing - Philips Hue
      hueCheckPaired: (ip: string) => Promise<{ paired: boolean; bridgeName?: string; username?: string }>;
      hueStartPairing: (ip: string) => Promise<{ status: string; message: string; actionRequired?: string; ip?: string; expiresAt?: number }>;
      hueCompletePairing: (ip: string) => Promise<{ status: string; message: string; username?: string }>;
      hueGetLights: (ip: string) => Promise<{ lights?: any; error?: string }>;
      hueSetLight: (ip: string, lightId: string, state: { on?: boolean; bri?: number; hue?: number; sat?: number }) => Promise<{ result?: any; error?: string }>;
      hueGetGroups: (ip: string) => Promise<{ groups?: any; error?: string }>;
      hueSetGroup: (ip: string, groupId: string, state: { on?: boolean; bri?: number; hue?: number; sat?: number }) => Promise<{ result?: any; error?: string }>;
      
      // Phase 6: The Reflex System - Instant Commands
      tryReflex: (text: string) => Promise<{ handled: boolean; response: string; latencyMs?: number; error?: boolean } | null>;
      
      // Phase 6: The Pulse - Real-time Device State
      getAllDeviceStates: () => Promise<Record<string, any>>;
      getAllDevices: () => Promise<any[]>;
      startDevicePolling: (intervalMs?: number) => Promise<{ success: boolean; interval?: number }>;
      stopDevicePolling: () => Promise<{ success: boolean }>;
      pollDevicesNow: () => Promise<Record<string, any>>;
      onDeviceStateUpdate: (callback: (states: Record<string, any>) => void) => () => void;
      
      // Phase 7: Audio Ducking
      duckAudio: (shouldDuck: boolean) => Promise<{ success: boolean; ducked?: boolean; level?: number; error?: string }>;
      
      // Phase 7: Screen Capture (Desktop Vision)
      captureScreen: (options?: { type?: string; activeOnly?: boolean; width?: number; height?: number }) => Promise<{
        success: boolean;
        image?: string;
        sourceName?: string;
        sourceId?: string;
      ...
      getCaptureSources: (options?: any) => Promise<{
        success: boolean;
        sources?: Array<{ id: string; name: string; thumbnail: string; isScreen: boolean }>;
        error?: string;
      }>;
      
      // Phase 7: Hardware Interrupt
      hardInterrupt: () => Promise<{ success: boolean; timestamp: number }>;
      onInterrupt: (callback: () => void) => () => void;
      
      // Phase 8: Native Sound Feedback
      playSound: (soundType: 'wake' | 'processing' | 'success' | 'error' | 'sleep' | 'interrupt') => Promise<{ success: boolean; soundType?: string; error?: string }>;
      
      // Phase 8: Piper TTS (Neural Voice)
      speakPiper: (text: string, options?: { speed?: number }) => Promise<{ success: boolean; fallback?: boolean; reason?: string; error?: string }>;
      stopPiper: () => Promise<{ success: boolean }>;
      piperAvailable: () => Promise<{ available: boolean; voices: Array<{ id: string; name: string; path: string }> }>;
      setPiperVoice: (voiceId: string) => Promise<{ success: boolean }>;
    };
...
export class BrowserHardware {
   ...
   */
  static async hueCheckPaired(ip: string): Promise<{ paired: boolean; bridgeName?: string; username?: string }> {
    if (!window.jarvisHost?.hueCheckPaired) {
      return { paired: false };
    }
    return window.jarvisHost.hueCheckPaired(ip);
  }
  
  /**
   * Start Hue Bridge pairing - user needs to press link button
   */
  static async hueStartPairing(ip: string): Promise<{ status: string; message: string; actionRequired?: string }> {
    if (!window.jarvisHost?.hueStartPairing) {
      return { status: 'failed', message: 'Pairing only available in desktop mode' };
    }
    return window.jarvisHost.hueStartPairing(ip);
  }
  
  /**
   * Complete Hue pairing after button press
   */
  static async hueCompletePairing(ip: string): Promise<{ status: string; message: string; username?: string }> {
    if (!window.jarvisHost?.hueCompletePairing) {
      return { status: 'failed', message: 'Pairing only available in desktop mode' };
    }
    return window.jarvisHost.hueCompletePairing(ip);
  }
  
  /**
   * Get all lights from a paired Hue Bridge
   */
  static async hueGetLights(ip: string): Promise<{ lights?: any; error?: string }> {
    if (!window.jarvisHost?.hueGetLights) {
      return { error: 'Not running in desktop mode' };
    }
    return window.jarvisHost.hueGetLights(ip);
  }
  
  /**
   * Control a Hue light
   */
  static async hueSetLight(ip: string, lightId: string, state: { on?: boolean; bri?: number; hue?: number; sat?: number }): Promise<{ result?: any; error?: string }> {
    if (!window.jarvisHost?.hueSetLight) {
      return { error: 'Not running in desktop mode' };
    }
    return window.jarvisHost.hueSetLight(ip, lightId, state);
  }
  
  /**
   * Get all rooms/groups from a paired Hue Bridge
   */
  static async hueGetGroups(ip: string): Promise<{ groups?: any; error?: string }> {
    if (!window.jarvisHost?.hueGetGroups) {
      return { error: 'Not running in desktop mode' };
    }
    return window.jarvisHost.hueGetGroups(ip);
  }
  
  /**
   * Control a Hue room/group
   */
  static async hueSetGroup(ip: string, groupId: string, state: { on?: boolean; bri?: number; hue?: number; sat?: number }): Promise<{ result?: any; error?: string }> {
    if (!window.jarvisHost?.hueSetGroup) {
      return { error: 'Not running in desktop mode' };
    }
    return window.jarvisHost.hueSetGroup(ip, groupId, state);
  }
  ...
  /**
   * Try to execute a command as a reflex (instant, no LLM)
   * Returns { handled: boolean, response: string, latencyMs: number } or null
   */
  static async tryReflex(text: string): Promise<{ handled: boolean; response: string; latencyMs?: number; error?: boolean } | null> {
    if (!window.jarvisHost?.tryReflex) {
      return null;
    }
    return window.jarvisHost.tryReflex(text);
  }
   ...
   */
  static async getAllDeviceStates(): Promise<Record<string, any>> {
    if (!window.jarvisHost?.getAllDeviceStates) {
      return {};
    }
    return window.jarvisHost.getAllDeviceStates();
  }
  ...
  static async getAllDevices(): Promise<any[]> {
    if (!window.jarvisHost?.getAllDevices) {
      return [];
    }
    return window.jarvisHost.getAllDevices();
  }
  
  /**
   * Start real-time device polling
   */
  static async startDevicePolling(intervalMs: number = 2000): Promise<{ success: boolean }> {
    if (!window.jarvisHost?.startDevicePolling) {
      return { success: false };
    }
    return window.jarvisHost.startDevicePolling(intervalMs);
  }
  
  /**
   * Stop device polling
   */
  static async stopDevicePolling(): Promise<{ success: boolean }> {
    if (!window.jarvisHost?.stopDevicePolling) {
      return { success: false };
    }
    return window.jarvisHost.stopDevicePolling();
  }
  
  /**
   * Force an immediate device state poll
   */
  static async pollDevicesNow(): Promise<Record<string, any>> {
    if (!window.jarvisHost?.pollDevicesNow) {
      return {};
    }
    return window.jarvisHost.pollDevicesNow();
  }
   ...
   */
  static onDeviceStateUpdate(callback: (states: Record<string, any>) => void): () => void {
    if (!window.jarvisHost?.onDeviceStateUpdate) {
      return () => {};
    }
    return window.jarvisHost.onDeviceStateUpdate(callback);
  }
}
