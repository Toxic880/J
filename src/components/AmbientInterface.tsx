 ...
 * Arc Reactor dims/changes color based on system health
 */
...
import { healthMonitor, HealthState, ServiceStatus } from '../services/HealthMonitor';
...
export const AmbientInterface: React.FC = () => {
  ...
  // === PHASE 9: HEALTH STATE ===
  const [healthStatus, setHealthStatus] = useState<ServiceStatus>('healthy');
  
  // Subscribe to health monitor
  useEffect(() => {
    const unsubscribe = healthMonitor.subscribe((healthState: HealthState) => {
      setHealthStatus(healthState.overall);
    });
    return unsubscribe;
  }, []);
  
  // === PHASE 12: POWER STATE (invisible to user but affects UI subtly) ===
  const [powerTier, setPowerTier] = useState<'SLEEP' | 'LIGHT' | 'FULL'>('LIGHT');
  
  useEffect(() => {
    // Import dynamically to avoid circular deps
    import('../services/PowerManager').then(({ powerManager }) => {
      const unsubscribe = powerManager.subscribe((powerState) => {
        setPowerTier(powerState.currentTier);
      });
      return unsubscribe;
    });
  }, []);
  
  // === PHASE 11: WAKE DETECTION VISUAL FEEDBACK ===
  const [wakeDetected, setWakeDetected] = useState(false);
  
  // Listen for wake word detection events
  useEffect(() => {
    const handleWakeDetected = () => {
      console.log('[UI] Wake word detected - flashing Arc Reactor');
      setWakeDetected(true);
      
      // Reset after 500ms - the flash is brief but obvious
      setTimeout(() => {
        setWakeDetected(false);
      }, 500);
    };
    
    // Listen for custom event from SpeechService/JarvisCore
    window.addEventListener('jarvis-wake-detected', handleWakeDetected);
    
    // Also trigger on status change to LISTENING (backup)
    if (state?.status === 'LISTENING' && !wakeDetected) {
      handleWakeDetected();
    }
    
    return () => {
      window.removeEventListener('jarvis-wake-detected', handleWakeDetected);
    };
  }, [state?.status]);
      ...
      <div className="absolute inset-0 flex items-center justify-center z-10">
        <div 
          ...
          className={`
            ...
            ${powerTier === 'SLEEP' ? 'opacity-40 scale-95' : ''}
            hover:scale-105
          `}
        >
          <ArcReactor 
            volume={ui.volume} 
            status={powerTier === 'SLEEP' ? 'STANDBY' : state.status} 
            healthStatus={healthStatus} 
            wakeDetected={wakeDetected} 
          />
        </div>
        
        {/* Phase 12: Subtle power tier indicator (only in sleep mode) */}
        {powerTier === 'SLEEP' && !hasOverlay && (
