...
export interface UIState {
  ...
  // Phase 6: Real-time device states from The Pulse
...
export function JarvisProvider({ children }: { children: React.ReactNode }) {
  ...
  const [ui, setUi] = useState<UIState>({
    ...
    deviceStates: {}, // Phase 6: Real-time device states
  });
  ...
  // ===========================================================================
  // PHASE 6: THE PULSE - Real-time Device State Listener
  // ===========================================================================
  useEffect(() => {
    // Only set up listener if we're in Electron with jarvisHost
    if (!window.jarvisHost?.onDeviceStateUpdate) {
      return;
    }
    
    console.log('[JarvisContext] Setting up device state listener (The Pulse)');
    
    // Subscribe to device state updates from Electron
    const unsubscribe = window.jarvisHost.onDeviceStateUpdate((states) => {
      console.log('[JarvisContext] Device state update:', Object.keys(states).length, 'devices');
      setUi(prev => ({ ...prev, deviceStates: states }));
    });
    
    // Initial load of device states
    window.jarvisHost.getAllDeviceStates?.().then(states => {
      if (states && Object.keys(states).length > 0) {
        console.log('[JarvisContext] Initial device states loaded:', Object.keys(states).length);
        setUi(prev => ({ ...prev, deviceStates: states }));
      }
    });
